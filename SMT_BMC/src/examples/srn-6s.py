...
def GetStep(step):
    ...
    total_rate = Real('total_rate.{0}'.format(step-1))
    total_rate = (ToReal(s1)*ToReal(s2)) + ToReal(s3) + (ToReal(s3)*0.1) + (ToReal(s4)*ToReal(s5)) + ToReal(s6) + (ToReal(s6)*0.1)

    ranges = And(0 <= trp, trp <= 1)
   
    transition_1 = And(trp==(ToReal(s1)*ToReal(s2))/total_rate, s1_nxt==(s1 - 1), s2_nxt==(s2 - 1), s3_nxt==(s3 + 1), s4_nxt==(s4), s5_nxt==(s5),s6_nxt==(s6))
    transition_2 = And(trp==(ToReal(s3)/total_rate), s1_nxt==(s1 + 1), s2_nxt==(s2 + 1), s3_nxt==(s3 - 1), s4_nxt==(s4), s5_nxt==(s5),s6_nxt==(s6))
    transition_3 = And(trp==(ToReal(s3)*0.1)/total_rate, s1_nxt==(s1+1), s2_nxt==(s2), s3_nxt==(s3-1), s4_nxt==(s4), s5_nxt==(s5+1), s6_nxt==(s6))
    transition_4 = And(trp==(ToReal(s4)*ToReal(s5))/total_rate, s1_nxt==(s1), s2_nxt==(s2), s3_nxt==(s3), s4_nxt==(s4-1), s5_nxt==(s5-1), s6_nxt==(s6+1))
    #transition_4 = And(trp==total_rate, s1_nxt==(s1), s2_nxt==(s2), s3_nxt==(s3), s4_nxt==(s4-1), s5_nxt==(s5-1), s6_nxt==(s6+1))
    transition_5 = And(trp==ToReal(s6)/total_rate, s1_nxt==(s1), s2_nxt==(s2), s3_nxt==(s3), s4_nxt==(s4+1), s5_nxt==(s5+1), s6_nxt==(s6-1))
    transition_6 = And(trp==(ToReal(s6)*0.1)/total_rate, s1_nxt==(s1), s2_nxt==(s2+1), s3_nxt==(s3), s4_nxt==(s4+1), s5_nxt==(s5), s6_nxt==(s6-1))
   
    #step = And(ranges, Or(transition_1, transition_2, transition_3, transition_4, transition_5, transition_6)) #Note takes the value after s5_nxt==(s5-1)
    step = Or(transition_1, transition_2, transition_3, transition_4, transition_5, transition_6) #Note takes the value after s5_nxt==(s5-1)
