...
class BMC:
    ...
    def Print(self, modelName):
        ...
        try: # Adding the last node "T" which holds the property
            x = self.priorTerminalValues.index(str(subString))
            if(self.graph.searchTerminalEdges(str(previousEdge) + " " + ("T" + str(self.T)), str(-1)) == 0): # Search for redundant edges.
            ...
            self.priorTerminalValues.append(str(subString))
            if(self.graph.searchTerminalEdges(str(previousEdge) + " " + ("T" + str(self.T)), str(-1)) == 0): # Search for redundant edges.
                self.graph.addTerminalEdge(str(previousEdge) + " " + ("T" + str(self.T)), str(-1)) # Adding last edge.
