...
def BMC(path, path_length):
    ...
    try:  # Only ran if a counter-example was found
        while(True):  # Calculate the probability of all counter examples that can be generated
            ...
            new_model = Not(And(cx_value_list))  # Counter example to begin avoiding
            new_model_list.append(new_model)  # Add to list of counter examples to avoid
            all_new_model_constraints = And(new_model_list)  # All of the counter examples to avoid
...
property_prob = (float(last_line.split("=", 1)[1]))  # Retrieve the property probability threshold
