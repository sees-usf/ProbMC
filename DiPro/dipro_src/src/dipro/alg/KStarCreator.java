...
public class KStarCreator implements SearchCreator {

	@Override
	public BF createSearch(Context context) throws Exception {
		KStar alg = new KStar();
		Comparator<Double> comparator = new DoubleComparator();
		KEvaluationFunction evaluationFunction = new KEvaluationFunction(
				alg);
	...
	public BF createSearch(Context context, boolean lengthHeuristic)
		...
		{
			Comparator<Double> comparator = new DoubleComparator();
			EvaluationFunctionLength evaluationFunction = new EvaluationFunctionLength(
				alg);
			alg.bind(context, comparator, evaluationFunction);
		}
		else
		{
			alg = (KStar) createSearch(context);
		}
		return alg;
	}
