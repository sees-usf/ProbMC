...
public class Heap_BAK<E> {
	 ...
	 */
	public Heap_BAK(int initialCapacity) {
	 ...
	 *             if {@code initialCapacity} is less than 1
	 */
	public Heap_BAK(int initialCapacity, Comparator<? super E> comparator) {
	...
	public HeapNode nodeOf(E e) {
		int i = indexOf(e);
		if (i < 0)
			return null;
		return new HeapNode(i);
	}
	...
	private int indexOfEq(Object o) {
		if (o != null) {
			for (int i = 0; i < size; i++)
				if (o == queue[i])
					return i;
	 ...
	 */
	public Heap_BAK<E> copy() {
		Heap_BAK<E> cpy = new Heap_BAK<E>(1, this.comparator);
		cpy.queue = Arrays.copyOf(this.queue, this.queue.length);
		cpy.modCount = this.modCount;
		cpy.size = this.size;
		return cpy;
	}
	...
	public class HeapNode {
		...
		@SuppressWarnings("unchecked")
		private HeapNode(int index) {
			assert index >= 0 && index < size;
			this.index = index;
			this.element = (E) queue[index];
		}
		...
		public HeapNode getRight() {
			...
			if (rightIndex >= size)
				return null;
			return new HeapNode(rightIndex);
		}
